...
export default function StaffAttendancePage() {
    ...
    useEffect(() => {
        fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://demo-school-soxa.onrender.com"}/hrm/departments`).then(r => r.json()).then(setDepartments).catch(() => { });
    }, []);
    ...
    const loadAttendance = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const url = `${process.env.NEXT_PUBLIC_API_URL || "https://demo-school-soxa.onrender.com"}/attendance/staff/daily?date=${date}${deptId ? `&department_id=${deptId}` : ''}`;
    ...
    const saveAttendance = async () => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://demo-school-soxa.onrender.com"}/attendance/staff/daily`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ date, records }) });
