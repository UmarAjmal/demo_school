...
export default function StudentAttendanceHistoryPage() {
    ...
    useEffect(() => {
        fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://demo-school-soxa.onrender.com"}/academic`).then(r => r.json()).then(setClasses).catch(() => { });
    }, []);
    ...
    const loadHistory = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://demo-school-soxa.onrender.com"}/attendance/students/history?class_id=${classId}&month=${month}&year=${year}`);
